...
namespace {
  ...
  TEST(Tree, MultipleAddsDontInterfere) {
    ...
    EXPECT_EQ(t.match(ds::TriVec{"+"}),
              std::optional<ds::Trit>(ds::Trit::Plus));
    EXPECT_EQ(t.match(ds::TriVec{"0"}),
              std::optional<ds::Trit>(ds::Trit::Zero));
    EXPECT_EQ(t.match(ds::TriVec{"-"}),
              std::optional<ds::Trit>(ds::Trit::Minus));
  }
