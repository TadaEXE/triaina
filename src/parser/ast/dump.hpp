...
class Dumper {
 public:
  ...
  void dump(const Program& p) {
    ...
    for (size_t i = 0; i < p.item_list.size(); ++i) {
      branch("item[" + std::to_string(i) + "]",
             i + 1 == p.item_list.size(),
             [&] { dump(p.item_list[i]); });
    }
  }
 ...
 private:
  ...
  void dump(const Input& n) {
    leaf("Input " + n.identifier +
         " trigger='" + std::string(1, n.trigger.c) + "'",
         true);
  }

  void dump(const Output& n) {
    leaf("Output " + n.identifier +
         " label=\"" + n.name.str + "\"",
         true);
  }
  ...
  void dump(const Gate& n) {
    branch("Gate " + n.identifier +
               " width=" + std::to_string(n.width.dec),
           true,
           [&] {
             for (size_t i = 0; i < n.gate_arm_list.size(); ++i) {
               dump(n.gate_arm_list[i], i,
                    i + 1 == n.gate_arm_list.size());
             }
           });
  }

  void dump(const GateArm& n,
            size_t idx,
            bool is_last) {
    leaf("arm[" + std::to_string(idx) + "]: " +
             pattern(n.trit_match_list) +
             " >> " + trit(n.result.trit),
         is_last);
  }

  void dump(const Node& n) {
    branch("Node " + n.identifier,
           true,
           [&] {
             /* ports */
             branch("ports", false, [&] {
               for (size_t i = 0; i < n.port_list.size(); ++i) {
                 leaf(port(n.port_list[i]),
                      i + 1 == n.port_list.size());
               }
             });
             ...
             branch("connections", true, [&] {
               for (size_t i = 0; i < n.chain_list.size(); ++i) {
                 dump(n.chain_list[i], i,
                      i + 1 == n.chain_list.size());
               }
             });
           });
  }
  ...
  void dump(const Chain& c,
            size_t idx,
            bool is_last) {
    branch("chain[" + std::to_string(idx) + "]",
           is_last,
           [&] {
             for (size_t i = 0; i < c.elements.size(); ++i) {
               leaf(expr(c.elements[i]),
                    i + 1 == c.elements.size());
             }
           });
  }
  ...
  std::string expr(const Qualified& q) {
    return q.node + "." + q.property;
  }
  ...
  std::string expr(const TritLiteral& t) {
    return trit(t.trit);
  }
