...
class pseudo_graph {
 public:
  ...
  template <typename T> bool remove (T* ptr) {
    auto& v = inner<T> ();
  ...
  template <typename T> T* find (uintptr_t id) {
    auto& v = inner<T> ();
    auto it =
    std::find_if (v.begin (), v.end (), [id] (auto* i) { return i->id () == id; });
 ...
 public:
  ...
  const auto port_count () const {
    return _ports.size ();
  }

 private:
  template <typename T> constexpr auto& inner () {
