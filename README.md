...
`foreach` is used to look at each individual trit in a trit-vector. It's designed to allow trit-wise operations with context, as gates have none between individual trits of the operands.
...
func cat:?n(lhs:?, rhs:?) {
    tmp:n = rhs;
    for x, tmp
    return or(lhs, rhs); // exploit automatic extension
}
```
